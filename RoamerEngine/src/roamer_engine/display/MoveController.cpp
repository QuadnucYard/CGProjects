...
namespace qy::cg {
	...
	void MoveController::update() {
		...
		/*auto roll = glm::eulerAngles(rotNew).z;
		if (!((roll > -0.01 && roll<0.01) || roll > 3.13 || roll <-3.13)) {
			auto rotRoll = glm::quat {cos(-roll), sin(-roll) * glm::vec3 {0, 0, 1}};
			rotNew = rotRoll * rotNew;
		}*/
